...
#DIR = '/Volumes/Scratch/Scanning/Apple/Apple Service Technical Procedures Macintosh Family Volume One/pages600'
DIR = '/Volumes/Scratch/Scanning/Apple/Apple Service Technical Procedures Cross Family Peripherals Volume Two/pages600'
        ...
        if thinnest and aspectmax > 60:
            ...
            if True:
                ...
                if angles:
                    ...
                    eprint("  no lines detected")
                imsave('out/{}_{}_edges.png'.format(i, a), edgesg)
            else:
                # This approach just doesn't work very well
                edges = canny(pos[row-100:row+100], 2)
                h, a, d = hough_line(edges, theta=arange(deg2rad(-3.0), deg2rad(3.0), deg2rad(0.02)))
                h2, a2, d2 = hough_line_peaks(h, a, d, min_distance=pagew/2)

                a = rad2deg(median(a2))
                eprint("  angle: {} deg".format(a))
                imsave('out/{}_{}_edges.png'.format(i, a), edges)
    else:
        eprint("  Did not find rule inside image margin")
